...
public class TPOActivity extends AppCompatActivity {
    ...
    private String parentDbName = "TPO";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_tpo);

        inputId = (EditText) findViewById(R.id.editText8);
        inputPassword = (EditText) findViewById(R.id.editText9);
        btnSignin = (Button) findViewById(R.id.button5);
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.child(parentDbName).child(id).exists()) {
                            if(dataSnapshot.child(parentDbName).child(id).child("Password").getValue().toString().equals(password)){
                                Toast.makeText(TPOActivity.this, "logged in Successfully", Toast.LENGTH_SHORT).show();
                                Intent i = new Intent(TPOActivity.this, TPOHomeActivity.class);
                                startActivity(i);
                            }else{
                                Toast.makeText(TPOActivity.this, "Enter correct password!", Toast.LENGTH_SHORT).show();
                            }
                        } else {
